'''
СКРИПТ ДЛЯ РАСЧЁТА МЕТРИК
В ФУНКЦИИ MAIN УКАЖИТЕ ПУТИ ДО МАСОК И ПРЕДИКТОВ И ЗПУСТИТЕ СКРИПТ
'''
...
"""
УКАЖИТЕ ПУТИ ДО МАСОК И ИЗОБРАЖЕНИЙ
"""
def main():

    masks_path = 'train/my_masks'
    preds_path = 'train/my_preds'

    f1_water = calculate_f1_score(masks_path, preds_path)

    osm_path = 'train/osm/9.geojson'

    pre_gt_path = 'train/my_masks/9_1.tif'
    post_gt_path = 'train/my_masks/9_2.tif'

    pre_pred_path = 'train/my_preds/merged_image_1.tif'
    post_pred_path = 'train/my_preds/merged_image_2.tif'
    ...
    print(f"F1-Score water: {f1_water :.3f}")
    print(f"F1-Score business: {avg_f1_business :.3f}")
